    ...
    private func startConnection(generation: UInt64, service: any PulseCallServing, attempt: Int) {
        ...
        connectionTask = Task { [weak self] in
            do {
                ...
                let owner = self
                let call = try await self.realtime.beginCall(credential: credential, configuration: .init(), executor: executor, initialContext: "<estado_inicial_moa>\n\(overview.output)\n</estado_inicial_moa>", onState: { [weak owner] event in let o = owner; Task { @MainActor in o?.apply(event, generation: generation, service: service, attempt: attempt) } }, onText: { [weak owner] text in let o = owner; Task { @MainActor in o?.append(text, owner: false, generation: generation) } }, onAudio: { [weak owner] pcm in let o = owner; Task { @MainActor in guard o?.owns(generation) == true else { return }; o?.voice.playPCM16(pcm) } })
