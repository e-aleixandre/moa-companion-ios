...
public enum PulseRealtimePrompt {
    ...
    Al describir una sesión, resume en una sola frase natural su objetivo y cómo va, usando intenta y va junto con el estado vivo (preguntas pendientes, permisos pendientes y estado). No enumeres herramientas ni comandos salvo que el propietario lo pida explícitamente, por ejemplo «¿qué comando ha lanzado?». Si el brief no tiene actualización reciente o no la tiene, dilo con honestidad y ofrece leer el último mensaje de la sesión.
