    ...
    func testDurableRealtimeBudgetReservationsAreAtomicAndRecoverAcrossRestart() {
        ...
        for ledger in [firstLedger, secondLedger] {
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                ready.signal()
                start.wait()
                if let reservation = ledger.reserve(amountUSD: 0.6, budget: budget) {
                    resultsLock.lock()
        ...
        let recoveredActive = recovered.activeReservations()
        XCTAssertEqual(recoveredActive.count, 1, "restart keeps the persisted active reservation")
        let rejected = recovered.reserve(amountUSD: 0.5, budget: budget)
        XCTAssertNil(rejected, "recovered reservation still constrains the cap")
    }

    func testRealtimeBudgetSettlesKnownOnceAndRetainsUnknownUntilNextDay() {
        let suite = "PulseRealtimeBudgetTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suite)!; defer { defaults.removePersistentDomain(forName: suite) }
        ...
        let known = ledger.reserve(amountUSD: 0.5, budget: budget, now: now)!
        ledger.markRequestSent(turnID: known)
        ledger.settle(turnID: known, knownCostUSD: 0.2)
        ledger.settle(turnID: known, knownCostUSD: 0.2)
        let knownTotals = ledger.totals(now: now)
        XCTAssertEqual(knownTotals.day, 0.2, "a duplicated done event cannot double count")
        let unknown = ledger.reserve(amountUSD: 0.5, budget: budget, now: now)!
        ledger.markRequestSent(turnID: unknown)
        let unknownActive = ledger.activeReservations(now: now)
        XCTAssertEqual(unknownActive.count, 1)
        let tomorrow = now.addingTimeInterval(86_400)
        let expiredActive = ledger.activeReservations(now: tomorrow)
        XCTAssertEqual(expiredActive.count, 0)
        // The unknown amount is settled on its original day, never zeroed.
        let expiredTotals = ledger.totals(now: now)
        XCTAssertEqual(expiredTotals.day, 0.7)
    }

    func testRealtimeBudgetReleasesOnlyFailedPreSendAndSessionRotationIsExplicit() {
        let suite = "PulseRealtimeBudgetTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suite)!; defer { defaults.removePersistentDomain(forName: suite) }
        let ledger = PulseRealtimeBudgetLedger(store: .init(defaults: defaults, key: "ledger"))
        let budget = PulseRealtimeBudget(perSessionHardUSD: 1, perDayHardUSD: 2)
        let preSend = ledger.reserve(amountUSD: 0.6, budget: budget)!
        ledger.releaseIfPreSend(turnID: preSend)
        let releasedActive = ledger.activeReservations()
        XCTAssertEqual(releasedActive.count, 0)
        let postSend = ledger.reserve(amountUSD: 0.6, budget: budget)!
        ledger.markRequestSent(turnID: postSend)
        ledger.releaseIfPreSend(turnID: postSend)
        let retainedActive = ledger.activeReservations()
        XCTAssertEqual(retainedActive.count, 1, "a post-send drop remains conservatively reserved")
        ledger.rotateSession()
        let rotationRejected = ledger.reserve(amountUSD: 0.6, budget: budget)
        XCTAssertNil(rotationRejected, "rotation never drops an active reservation")
    }

    func testRealtimeBudgetEnforcesDailyHardLimitIndependentlyOfSessionLimit() {
        let suite = "PulseRealtimeBudgetTests.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suite)!; defer { defaults.removePersistentDomain(forName: suite) }
        let ledger = PulseRealtimeBudgetLedger(store: .init(defaults: defaults, key: "ledger"))
        let budget = PulseRealtimeBudget(perSessionHardUSD: 5, perDayHardUSD: 1)
        let accepted = ledger.reserve(amountUSD: 0.6, budget: budget)
        let rejected = ledger.reserve(amountUSD: 0.5, budget: budget)
