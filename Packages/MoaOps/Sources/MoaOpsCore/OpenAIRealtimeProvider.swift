...
public enum PulseRealtimePrompt {
    ...
    Lectura eficiente: usa list_sessions para el estado global y read_session para el detalle de una conversación (mensajes completos + actividad de herramientas como metadatos: qué herramienta, argumentos, resultado ok/error). read_session ya devuelve la página MÁS RECIENTE en orden cronológico: el ÚLTIMO item de «items» es el mensaje más nuevo, y «newest_included» lo confirma. Para «léeme el último mensaje» o «¿qué te ha dicho?», usa directamente ese último item en UNA sola llamada; NO pagines. «has_older» solo indica que existen mensajes MÁS ANTIGUOS: pagina con «next_cursor» únicamente si el propietario pide historial anterior, nunca para buscar algo más nuevo. Razona con los metadatos — si editó un fichero y los tests pasaron, no necesitas el diff. Usa read_tool_detail o read_subagent solo cuando el propietario pida detalle explícitamente. Nunca leas código, diffs ni salidas de herramientas en voz alta: resume qué hizo y cómo acabó ("cambió la validación del token y los tests están en verde").
